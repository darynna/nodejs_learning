...
exports.getTodos = async (query, owner) => {
  ...
  // search features
  // const todos = await Todo.find({ title: { $regex: query.search, $option: 'i' } });
  ...
  if (query.search && owner.role === userRolesEnum.USER) {
    for (const searchOption of findOptions.$or) {
      searchOption.owner = owner;
    }
  }
  ...
  // order ASC or DESC
  // todosQuery.sort('title' | '-teest);
  todosQuery.sort(`${query.search === 'DESC' ? '-' : ''}${query.sort ?? 'title'}`);
  ...
  const paginationLimit = query.limit ? +query : 5;
